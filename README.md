...
Программа для изменения громкости процессов или постановке воспроизведения на паузу по нажатию на горячую клавишу. / A program for changing the volume of processes or pausing playback by pressing a hotkey.
...
Играя в сессионные игры в одиночку, я часто включал на фон видео из браузера, однако были нередки моменты, когда необходимо прислушаться к происходящему в игре, а сворачивать приложение и ставить видео на паузу не очень удобно и забирает немало драгоценного времени. При помощи этой программы можно нажатием одной клавиши изменить громкость только у необходимых процессов или поставить воспроизведение контента на паузу.

When playing session games alone, I often included videos from the browser in the background, but there were often times when I needed to listen to what was happening in the game, and minimizing the application and pausing the video was not very convenient and took up a lot of precious time. With this program, you can press a single key to change the volume of only the necessary processes or pause the playback of content.
...
В директории откуда запускается программа могут находиться два файла "[mute.mp3](https://github.com/hlhpage/mute-this-please/blob/master/mute.mp3)" и "[unmute.mp3](https://github.com/hlhpage/mute-this-please/blob/master/unmute.mp3)", которые используются для звуковой индикации при работе программы. Потенциально вы можете использовать свои звуки, однако в качестве примера в корневой директории репозитория есть примеры таких файлов. Если билдите программу, то поместите эти файлы в директорию билда иначе при запуске будет появляться окно с предупреждением. Так же локализации на разные языки лежат в папке "[languages](https://github.com/hlhpage/mute-this-please/tree/master/languages)" в директории откуда запускается программа, однако если ни одного файла локализации внутри не будет, то программа автоматически создаст файл локализации на русском языке, но препятствием для запуска отсутствие файлов локализаций не будет. Тоже применимо к файлу с настройками "preferences.json". При его отсутствии программа создаст его в директории исполнения exe файла.

There may be two files in the directory from where the program is launched. "[mute.mp3](https://github.com/hlhpage/mute-this-please/blob/master/mute.mp3 )" and "[unmute.mp3](https://github.com/hlhpage/mute-this-please/blob/master/unmute.mp3 )", which are used for an audible indication when the program is running. You can potentially use your own sounds, however, as an example, there are examples of such files in the root directory of the repository. If you are building a program, then place these files in the build directory, otherwise a warning window will appear at startup. Localizations for different languages are also located in the "[languages](https://github.com/hlhpage/mute-this-please/tree/master/languages )" folder in the directory from where the program is launched, however, if there are no localization files inside, the program will automatically create a localization file in Russian, but the lack of localization files will not be an obstacle to launching. The same applies to the preferences settings file.json". If it is missing, the program will create it in the executable directory of the exe file.
...
__5 - Радиокнопка "Громкость"__ определяет режим работы программы. В громкости программа будет менять громкость процессов на основе списка фокусированных программ и выбранного режима белого или чёрного списка.

__6 - Радиокнопка "Пауза"__ определяет режим работы программы. В режиме паузы будет эмулироваться нажатие кнопки "Pause", которая ставит воспроизводимый контент на паузу. Но не все программы могут реагировать на её нажатие.

__7 - Радиокнопка "Чёрный список"__ определяет режим работы программы в режиме "Громкость". В режиме чёрного списка громкость будет меняться только у процессов, которые находятся в списке сфокусированных программ.
